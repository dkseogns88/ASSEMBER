...
void IOCompletionPort::WorkerThread()
{
	...
	while (m_bWorkerThread)
	{
		...
		else
		{
			...
			location loc;
			if (!strcmp(pSocketInfo->dataBuf.buf, "req_location"))
			{
				loc.x = 2000 + (rand() % (600 + 1));
				loc.y = (rand() % (500 + 1));
				loc.z = 500 + (rand() % (150 + 1));

				pSocketInfo->dataBuf.buf = (CHAR*)&loc;
				pSocketInfo->dataBuf.len = sizeof(loc);
			}
			...
			printf_s("[INFO] 메시지 송신 - Bytes : [%d], Msg : [x : %f, y : %f, z : %f]\n",
				pSocketInfo->dataBuf.len, loc.x, loc.y, loc.z);
