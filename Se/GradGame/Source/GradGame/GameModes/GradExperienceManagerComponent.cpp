...
void UGradExperienceManagerComponent::CallOrRegister_OnExperienceLoaded(FOnGradExperienceLoaded::FDelegate&& Delegate)
{
	...
	// 왜 맵핑 했을까?
	// 내가 물어보는 시점에 Experience가 로딩이 되어 있는지 안되어 있는지 알 수가 없다.
	// 따라서, 로딩이 되어 있으면 요청한 함수를 바로 실행하고
	// 아니면 Delegate를 추가해준다.
...
void UGradExperienceManagerComponent::ServerSetCurrentExperience(FPrimaryAssetId ExperienceId)
{
	...
	// 왜 CDO를 가져오는 걸까? : PrimaryAssetData(심화강의)
...
void UGradExperienceManagerComponent::StartExperienceLoad()
{
	...
	//   1. 우리는 B_GradDefaultExperience를 BP로 만든 이유: PrimaryAssetData(심화강의)
	//      - Data Asset 타입으로 만든다면 GetPrimaryAssetId 함수에서 우리가 원하는 타입이 아닌 다른 타입으로 가지고 오기 때문이다.
	//   2. CDO를 가져와서, GetPrimaryAssetId를 호출한 이유: PrimaryAssetData(심화강의)
	//      - GetPrimaryAssetId 함수 안을 보면 CDO 이어야만 진행이 되기 때문이다.
	...
	// 아래도, 후일 Bundle을 우리가 GameFeature에 연동하면서 더 깊게 알아보기로 하고, 지금은 앞서 B_GradDefaultExperience를 로딩해주는 함수로 생각하자
